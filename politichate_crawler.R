...
#parsing posts url----
all.url <- list()
pre.url <- "https://www.ptt.cc/bbs/HatePolitics/index%s.html"
for(i in 4064:2764){
  message(i)
...
for(i in c(0:2)){
  ...
  content.file.name = sprintf("hate_politics_content_4/hate_politics_content_%s.rds", i+1)
  push.file.name = sprintf("hate_politics_push_4/hate_politics_push_%s.rds", i+1)
...
for(url in all.url){
  tryCatch({
    ...
    if(length(css.pushtag) != 0){
      ...
      push.time <- html_text(css.pushtimeid)
      push.time <- substr(push.time, 2,  nchar(push.time)-1)
